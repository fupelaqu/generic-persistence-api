              ...
              state match {
                case Some(account) =>
                  ...
                  if(checkEncryption(credentials, oldPassword)){
                    val notifications = sendPasswordUpdated(generateUUID(), state.get)
                    Effect.persist[AccountEvent, Option[T]](
                      notifications.toList :+
                        PasswordUpdatedEvent(
                          entityId,
